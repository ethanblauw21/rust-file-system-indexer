...
- **CUDA GPU embedding (`cuda` feature, ADR-006)** — `cargo build --features cuda` registers the `ort` CUDA execution provider in `Embedder::load_inner` (`#[cfg(feature="cuda")]`, `.error_on_failure()` so a CUDA init failure is a named `IndexerError::Embedding`, never a silent CPU fallback; registration is inside the load timeout). Opt-in, never default — the CPU build stays portable and non-GPU CI is unaffected; `--features cuda` compiles without the CUDA SDK (`load-dynamic`). **Measured 24.9× end-to-end** on the reference GPU (full corpus 1h16m→3m5s). **Critical:** the GPU path MUST use the fp16 model (`onnx_model_file = "model_fp16.onnx"`) — the default int8-quantized export forces ~156 CPU-fallback memcpy nodes on the CUDA EP (~20× slower than CPU); fp16 runs on-GPU at cosine 1.0 vs fp32 (int8 is 0.96, i.e. fp16 is *more* faithful). Batch stays 32 (sweep-optimal; larger regresses on O(seq²) padding + 8 GB VRAM). Runtime: GPU onnxruntime 1.24.2 build + CUDA 12.x/cuDNN 9.x (pip `nvidia-*-cu12` wheels work, no admin). Full evidence + setup in `docs/adr/ADR-006-cuda-embedding-execution-provider.md` and README "GPU acceleration".
