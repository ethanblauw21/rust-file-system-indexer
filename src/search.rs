...
impl FusionWeights {
    ...
    /// RRF_*_WEIGHT / PATH_BOOST_WEIGHT / RRF_TRIGRAM_WEIGHT constants (computed once
    /// by the caller via `FusionWeights::from_config`).
    fn resolve(file_defaults: FusionWeights) -> Self {
        ...
        Self {
            ...
            trigram: parse("RRF_TRIGRAM_WEIGHT", file_defaults.trigram),
        }
    }
    ...
    fn from_config(raw: &crate::config::RawConfig) -> Self {
        Self {
            ...
            trigram: raw.fusion.trigram_weight.unwrap_or(RRF_TRIGRAM_WEIGHT),
        }
    }
}
...
pub async fn hybrid_search(
    ...
    let mut results = rrf_fuse(dense_res, sparse_res, trigram_res, query, FusionWeights::resolve(fusion_defaults));
